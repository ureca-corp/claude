...
`.claude/SESSION.md` 파일이 이미 존재하는지 확인한다.

**기존 SESSION.md가 있으면**:
1. 파일을 읽고 현재 Phase와 진행 상황을 파악한다
2. 사용자에게 이어서 진행할지 새로 시작할지 확인한다:
   ```
   "이전 세션이 발견되었습니다.
   현재 상태: Phase 1 진행 중 (배치 2 완료, 모호함 3개 남음)
   이어서 진행할까요, 아니면 새로 시작할까요?"
   ```
3. **이어서 진행**: 남은 모호함 목록을 복원하고 다음 배치부터 재개
4. **새로 시작**: SESSION.md를 덮어쓰고 Step 1부터 진행

**SESSION.md가 없으면**: Step 1부터 바로 진행한다.
...
명확화 과정에서 사용자가 언급한 개념들을 추출하고 그룹화하여 도메인을 정의한다.
특정 앱에 고정된 매핑을 쓰지 않고, 매번 사용자 입력에서 귀납적으로 도출한다.

**탐지 절차**:

```
1단계 — 핵심 명사 추출
  사용자 요청 + 모든 배치 답변에서 핵심 명사를 수집한다.
  예: "회원", "번역 기록", "미션", "추천 문장"
      "상품", "재고", "주문", "결제", "리뷰"
      "예약", "객실", "체크인"

2단계 — 개념 그룹화
  서로 밀접한 개념을 하나의 후보 도메인으로 묶는다.
  기준: "이 개념들이 별도 엔티티와 API를 가질 만큼 독립적인가?"
  예: 회원 + 프로필 + 인증     → 하나의 도메인
      상품 + 재고 + 카테고리  → 하나의 도메인
      주문                    → 하나의 도메인 (결제와 분리)

3단계 — 도메인명 부여
  각 그룹에 소문자 영문 도메인명을 붙인다.
  네이밍 가이드:
    - 핵심 엔티티를 복수형으로: users, products, orders
    - 행위 중심이면 명사화: payments, reservations, reviews
    - 한글 의미를 최대한 반영: 번역 기록 → translations
```

**예시 — 번역 앱**:
```
추출: 회원, 번역 기록, 미션, 추천 문장
그룹: [회원] [번역 기록] [미션] [추천 문장]
도메인: users, translations, missions, phrases
```

**예시 — 쇼핑몰**:
```
추출: 회원, 상품, 재고, 주문, 결제, 리뷰
그룹: [회원] [상품+재고] [주문] [결제] [리뷰]
도메인: users, products, orders, payments, reviews
```

**도메인 목록 반드시 사용자 확인**:

탐지 후 사용자에게 보여주고 수정 기회를 준다:
```
탐지된 도메인: users, translations, missions, phrases

"위 4개 도메인으로 진행할까요?
추가하거나 통합·분리할 도메인이 있으면 알려주세요."
```
