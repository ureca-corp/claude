...
DDD의 핵심: 각 도메인을 **Aggregate Root**를 중심으로 모델링한다.

### Aggregate Root란?

Aggregate Root는 해당 도메인의 "관문" 역할을 하는 핵심 엔티티다.
외부 도메인은 항상 Aggregate Root를 통해서만 접근하며, 내부 엔티티에 직접 접근하지 않는다.

```
✅ 올바른 접근:
  주문(Order) → 주문항목(OrderItem)
  결제 도메인은 Order를 통해 OrderItem에 접근

❌ 잘못된 접근:
  결제 도메인이 OrderItem에 직접 접근 (Aggregate Root를 건너뜀)
```

### Aggregate Root 식별 기준

Phase 2 인터뷰에서 수집한 정보를 바탕으로 아래 기준으로 판단한다:

1. **생명주기 주체**: 다른 엔티티의 생성·삭제를 담당하는가? ("A를 삭제하면 B도 삭제" → A가 Aggregate Root)
2. **외부 참조 대상**: 다른 도메인이 이 엔티티를 ID로 참조하는가?
3. **일관성 경계**: 이 엔티티와 하위 엔티티들이 항상 함께 변경되어야 하는가?
...
이 도메인의 Aggregate Root: **주문 (Order)**

- 주문항목(OrderItem), 주문상태(OrderStatus)는 Order를 통해서만 접근된다
- 외부 도메인은 Order ID로만 참조한다
```
...
domain-model.md 작성 중 아래 신호가 보이면 정규화 검토가 필요하다:

| 신호 | 문제 | 해결 |
|------|------|------|
| 같은 속성이 두 엔티티에 중복 저장 | 데이터 불일치 위험 | 공통 속성을 별도 엔티티로 분리 |
| 리스트 형태의 속성 (예: tag1, tag2, tag3) | 확장 불가 | 별도 관계 엔티티로 정규화 |
| 한 엔티티가 너무 많은 역할 담당 | 책임 과부하 | 역할별 엔티티 분리 |

정규화 검토 결과를 제약 조건 또는 관계 규칙 섹션에 반영한다.
