...
api-spec.md 파일 최상단에 ENUM 정의 섹션을 반드시 추가:
...
| message | 문자열 | 성공 메시지 | 회원가입이 완료되었습니다 |
| data | 객체 | 응답 데이터 | (아래 참조) |
...
| 필드명 | 타입 | 설명 | 예시 |
|--------|------|------|------|
| status | 문자열 | 오류 코드 (영문 대문자) | DUPLICATE_EMAIL |
| message | 문자열 | 오류 메시지 (한글) | 이미 가입된 이메일입니다 |
| data | null | 오류 시 항상 null | null |

#### 실패 (401 Unauthorized)

| 필드명 | 타입 | 설명 | 예시 |
|--------|------|------|------|
| status | 문자열 | 오류 코드 | UNAUTHORIZED |
| message | 문자열 | 오류 메시지 | 로그인이 필요합니다 |
| data | null | 오류 시 항상 null | null |
...
| 필드명 | 타입 | 설명 | 예시 |
|--------|------|------|------|
| status | 문자열 | 오류 코드 | ERROR |
| message | 문자열 | 오류 메시지 | 서버 오류가 발생했습니다 |
| data | null | 오류 시 항상 null | null |
```
...
|------|------|-----------|
| 200 | 성공 | 정상 처리 |
| 400 | 잘못된 요청 | 모든 클라이언트 오류 (권한, 리소스 없음, 중복, 검증 실패 등) |
| 401 | 인증 필요 | 인증 실패 |
| 5xx | 서버 오류 | 내부 오류 |

**중요**: 403, 404, 409, 422 등은 사용하지 않고 모두 400으로 통일합니다. 구체적인 오류는 `status` 필드의 영문 대문자 코드로 구분합니다.
